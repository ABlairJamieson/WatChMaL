...
class Digitizer:
    def __init__( self ):
        ...
        """
        self.noise_nmu       = 0.0
        self.noise_nsigma    = 0.5
        self.noise_ncomp     = 5
        self.noise_comprange = [1, 480]
        self.noise_compamp   = [0.05, 0.15]
    ...
    def digitize_event( self, truepmts, truetimes ):
        ...
        return ( digipmt, digicharge, digitime )
...
def f_waveform( t, U0=30, sigma0=0.3, tau0=30, U1=-1.2, sigma1=55, t1=-4, U2=-2.8, tau2=80):
    """
    Build waveform template for a single photon pulse as np.array of amplitudes. Based on arXiv:1801.08690
    Inputs:
    ...
    Uos1 = U1 * np.exp( -0.5 * ( ( dt-t1)/ sigma1 )**2 )
    Uos2 = U2 / ( np.exp( (50.0-dt)/10.0 ) + 1.0 ) * np.exp( -dt/tau2 )
    U = Upeak + Uos1 + Uos2
    return U
...
def plot_fitted_waveform( plot_title, twf, adcwf, tbin, window_width, params, adcwferr ):
    ...
    """
    adcwfexp = pulseshape( twf[ tbin-window_width: tbin+window_width], *params)
    resid = adcwf[ tbin-window_width: tbin+window_width] - adcwfexp
    chisq = np.sum((resid/adcwferr)**2)
    df    = window_width*2 - 4
    #print("chisq =",chisq,"df =",df)
    plt.figure(figsize=(10,10))
    plt.xlim( twf[tbin-window_width], twf[tbin+window_width])
    plt.errorbar( twf, adcwf, adcwferr*np.ones( len(adcwf)), fmt='.' )
    plt.xlabel('time (ns)')
    plt.ylabel('ADC')     
    plt.title( plot_title+" chi2/ndf = %d/%d"%(chisq,df) )
    tttmin = twf[ tbin-window_width*2 ]
    tttmax = twf[ tbin+window_width*2 ]
